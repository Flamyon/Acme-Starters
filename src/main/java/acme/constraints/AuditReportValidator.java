...
public class AuditReportValidator extends AbstractValidator<ValidAuditReport, AuditReport> {
	...
	public boolean isValid(final AuditReport auditReport, final ConstraintValidatorContext context) {
		...
		if (auditReport.getDraftMode() != null && !auditReport.getDraftMode()) {
			...
			{
				Integer existingAuditSections = this.repository.countAuditSectionsByAuditReportId(auditReport.getId());
				boolean hasSection = existingAuditSections != null && existingAuditSections > 0;

				super.state(context, hasSection, "draftMode", "acme.validation.auditReport.no-auditSections.message");
			}
		}
