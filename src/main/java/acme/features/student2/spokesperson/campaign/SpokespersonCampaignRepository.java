...
public interface SpokespersonCampaignRepository extends AbstractRepository {
	...
	@Query("select distinct p from ProjectMember pm join pm.project p where p.draftMode = true and pm.userAccount.id = :userAccountId and pm.roleKind = acme.entities.levelb.MemberRole.SPOKESPERSON order by p.title asc")
