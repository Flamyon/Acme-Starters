...
public class InventorProjectShowService extends AbstractService<Inventor, Project> {
	...
	public void authorise() {
		...
		isMember = this.project != null && this.project.getId() != 0 && this.repository.countProjectMemberByProjectIdAndRoleKindAndUserAccountId(this.project.getId(), userAccountId, MemberRole.INVENTOR) > 0;
	...
	public void unbind() {
		...
		userAccountId = super.getRequest().getPrincipal().getAccountId();
		isMember = this.project.getId() != 0 && this.repository.countProjectMemberByProjectIdAndRoleKindAndUserAccountId(this.project.getId(), userAccountId, MemberRole.INVENTOR) > 0;
