...
public interface InventorInventionRepository extends AbstractRepository {
	...
	@Query("select distinct p from ProjectMember pm join pm.project p where p.draftMode = true and pm.userAccount.id = :userAccountId and pm.roleKind = acme.entities.levelb.MemberRole.INVENTOR order by p.title asc")
